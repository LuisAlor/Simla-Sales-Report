...
const WAVES = [
  { cy: H * 0.30, amp: 38, period: 480, speed: "28s", opacity: 0.20, strokeW: 1.5, delay: "0s"  },
  { cy: H * 0.48, amp: 26, period: 360, speed: "20s", opacity: 0.14, strokeW: 1,   delay: "-7s" },
  { cy: H * 0.62, amp: 50, period: 560, speed: "34s", opacity: 0.24, strokeW: 2,   delay: "-4s" },
  { cy: H * 0.78, amp: 20, period: 300, speed: "16s", opacity: 0.11, strokeW: 1,   delay: "-12s"},
];
...
function LoginBackground() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden>
      ...
      {/* Base gradient */}
      <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 80% 60% at 50% 40%, #0d0d2e 0%, #060a14 70%)" }} />
      ...
      <svg className="absolute inset-0 w-full h-full" style={{ animation: "gridBreath 6s ease-in-out infinite" }}>
        <defs>
          <pattern id="lg-dot-grid" width="48" height="48" patternUnits="userSpaceOnUse">
            <circle cx="24" cy="24" r="1" fill="#8b5cf6" opacity="0.5" />
          </pattern>
        </defs>
      ...
      <div className="absolute inset-0" style={{
        background: "radial-gradient(ellipse 55% 55% at 50% 48%, rgba(139,92,246,0.08) 0%, transparent 70%)",
      }} />
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-sm mx-4">
        {/* Top glow line */}
        <div className="h-px rounded-t-xl" style={{ background: "linear-gradient(90deg, transparent, #8b5cf6, #6366f1, #3b82f6, transparent)" }} />

        <div className="bg-slate-900/75 backdrop-blur-xl border border-slate-700/40 rounded-b-xl rounded-tr-xl shadow-2xl p-8"
          style={{ boxShadow: "0 0 60px rgba(139,92,246,0.10), 0 25px 50px rgba(0,0,0,0.6)" }}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <button type="submit"
              ...
              style={{ background: "linear-gradient(135deg, #7c3aed 0%, #6366f1 50%, #3b82f6 100%)", boxShadow: "0 0 20px rgba(139,92,246,0.35)" }}>
              {t("login_submit")}
            </button>
          </form>
          ...
          <button
            ...
            className="mt-5 text-violet-400 hover:text-violet-300 text-xs transition-colors"
          >
            {t("login_forgot")}
          </button>
        </div>
