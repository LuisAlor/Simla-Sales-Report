...
export function ApiSetup() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setStatus("error");
      setErrorMsg(err instanceof Error ? err.message : t("apisetup_error"));
    }
  }
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-gray-900 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-slate-200 dark:border-gray-700 w-full max-w-md p-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <p className="font-bold text-slate-800 dark:text-slate-100">{t("apisetup_brand")}</p>
            <p className="text-slate-400 dark:text-slate-500 text-xs">{t("apisetup_tagline")}</p>
          </div>
        </div>

        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-1">{t("apisetup_title")}</h2>
        <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">{t("apisetup_desc")}</p>

        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label className="block text-slate-600 dark:text-slate-400 text-sm mb-1">{t("apisetup_label")}</label>
            <input
              type="password"
              value={apiKey}
              onChange={(e) => { setApiKey(e.target.value); setStatus("idle"); }}
              placeholder={t("apisetup_placeholder")}
              required
          ...
          >
            {status === "validating" ? t("apisetup_verifying") : t("apisetup_submit")}
          </button>
        </form>

        <p className="text-slate-400 dark:text-slate-500 text-xs mt-4 text-center">
          {t("apisetup_hint_prefix")} <strong>{t("apisetup_hint")}</strong>.
        </p>
      </div>
    </div>
  );
