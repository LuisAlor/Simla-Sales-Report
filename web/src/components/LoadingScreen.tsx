...
const TOTAL_STEPS = BARS.length + 6;
    ...
    <div className="flex flex-col items-center justify-center flex-1 min-h-[70vh] gap-6 select-none">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-slate-100 dark:border-gray-700 p-5 w-[340px]">
        ...
        <div className="flex items-end gap-1 h-28 relative z-10">
                ...
                <div
                  ...
                  style={{
                    ...
                    background: drawing
                      ...
                      : "transparent",
                    transition: drawing ? "height 0.12s ease-out" : drawn ? "background 0.3s ease" : "height 0.05s ease",
          ...
          <svg className="absolute inset-0 pointer-events-none" width="100%" height="100%" viewBox="0 0 280 112" preserveAspectRatio="none">
            {showLine && (
              <>
                <polyline points={POINTS_STR} fill="none" stroke="#F59E0B" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" opacity="0.2" strokeDasharray={LINE_LEN} strokeDashoffset={lineDash} />
                <polyline points={POINTS_STR} fill="none" stroke="#F59E0B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" strokeDasharray={LINE_LEN} strokeDashoffset={lineDash} />
                {lineProgress < 0.98 && (() => {
                  const idx = Math.min(Math.floor(lineProgress * (TREND.length - 1)), TREND.length - 2);
                  const tv = lineProgress * (TREND.length - 1) - idx;
                  const x = TREND[idx][0] + tv * (TREND[idx + 1][0] - TREND[idx][0]);
                  const y = TREND[idx][1] + tv * (TREND[idx + 1][1] - TREND[idx][1]);
        ...
        <div className="flex justify-between mt-2 px-0.5">
          {["Ene", "Feb", "Mar", "Abr", "May", "Jun"].map((m, i) => (
            <span key={m} className="text-[9px] text-slate-300 transition-colors duration-300" style={{ color: i * 2 < barsDrawn ? "#94A3B8" : "#E2E8F0" }}>{m}</span>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative">
          <svg width="72" height="72" viewBox="0 0 80 80">
            ...
            <circle cx="40" cy="40" r={r} fill="none" stroke="url(#loadGrad)" strokeWidth="6" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={dashOffset} transform="rotate(-90 40 40)" style={{ transition: "stroke-dashoffset 0.35s ease" }} />
            <defs>
