...
export function MultiSelect({ options, selected, onChange, placeholder = "Buscar…", emptyLabel = "Sin opciones", variant }: Props) {
  ...
  // if variant is explicitly set, honour it; otherwise follow the app theme
  const isDark = variant === "dark" ? true : variant === "light" ? false : themeIsDark;
    ...
    <div ref={containerRef} className="relative">
      ...
      <div className={triggerCls} onClick={openAndFocus}>
        {selectedOptions.length > 0 ? (
          ...
          <span className={`flex-1 text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>
            {placeholder}
          </span>
        )}
        <ChevronDown size={11} className={isDark ? "text-slate-500 shrink-0" : "text-slate-400 shrink-0"} />
      </div>

      {open && (
        <div className={`absolute top-full left-0 right-0 mt-1 border rounded-md z-50 shadow-lg ${
          isDark ? "bg-gray-900 border-gray-700/60" : "bg-white border-slate-200"
        }`}>
          {/* Selected chips (visible when multiple selected) */}
          {selectedOptions.length > 0 && (
            <div className={`flex flex-wrap gap-1 p-2 border-b ${isDark ? "border-gray-700/60" : "border-slate-100"}`}>
          ...
          {/* Search input */}
          <div className={`px-2 py-1.5 border-b ${isDark ? "border-gray-700/60" : "border-slate-100"}`}>
            <input
              ref={inputRef}
